  ...
  return (
    <div className={`relative flex flex-col justify-between p-6 h-[200px] rounded-[24px] ${bgColor} transition-transform hover:-translate-y-1`}>
      <div>
        ...
        <p className="text-sm font-medium text-slate-600 dark:text-slate-400 mt-1">{subtitle}</p>
      </div>
...
export function CurrentRoundWidget({ round, loading, liveStake }: CurrentRoundWidgetProps) {
    ...
    return (
      <Card className="h-full flex flex-col">
        <CardContent className="pt-6 flex-1">
          <div className="h-full w-full min-h-[160px] animate-pulse rounded-xl bg-slate-200 dark:bg-slate-800/50" />
        </CardContent>
      </Card>
    );
  }
  ...
  if (!round) {
    return (
      <Card className="flex items-center justify-center h-full min-h-[220px] border-dashed border-slate-300 dark:border-white/20 bg-transparent">
        <CardContent className="pt-6 flex flex-col items-center justify-center text-slate-500">
          <Power className="w-10 h-10 mb-4 opacity-50 text-indigo-500 dark:text-indigo-400" />
          <p className="font-medium tracking-wide">Initializing House Engine...</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="h-full min-h-[200px] bg-gradient-to-br from-[#1e1b4b] to-[#312e81] border-none shadow-[0_10px_40px_rgba(49,46,129,0.3)] dark:from-indigo-950 dark:to-slate-950 overflow-hidden relative rounded-[24px]">
      <div className="absolute -right-20 -top-20 h-40 w-40 rounded-full bg-white/10 blur-[50px]"></div>
      <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2 relative z-10">
        <div>
          <Badge variant="default" className="bg-white/10 hover:bg-white/20 text-white border-white/20 animate-pulse mb-2">
            PRO MODE
          </Badge>
      ...
      <CardContent className="relative z-10 pb-6">
        <div className="grid grid-cols-2 gap-3 mt-4">
          <div className="rounded-xl bg-white/10 p-3 backdrop-blur-md border border-white/10">
            <p className="text-[10px] uppercase font-bold text-indigo-200">Current Pool</p>
            <p className="text-lg font-black text-white mt-1">₹{((liveStake && liveStake > 0) ? liveStake : (round.totalStake || 0)).toLocaleString()}</p>
          </div>
          <div className="rounded-xl bg-white/10 p-3 backdrop-blur-md border border-white/10">
            <p className="text-[10px] uppercase font-bold text-indigo-200">Time Left</p>
...
function RoundMetric({ label, value, highlight, icon }: { label: string; value: string; highlight?: boolean; icon?: React.ReactNode }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white/60 dark:border-white/10 dark:bg-black/20 p-4 transition-all hover:bg-white/90 dark:hover:bg-black/40 flex flex-col justify-between backdrop-blur-md">
      ...
      <CardContent>
        {loading ? (
          <div className="space-y-3">
            {[0, 1, 2].map((item) => (
              <div key={item} className="h-16 animate-pulse rounded-xl bg-slate-200 dark:bg-slate-800" />
            ))}
          </div>
        ) : bets.length === 0 ? (
          ...
          <div className="space-y-3">
            {bets.slice(0, 5).map((bet) => (
              <div
                key={bet.id}
                className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900/50 p-4 transition-all hover:border-slate-300 hover:bg-slate-100 dark:hover:border-slate-700 dark:hover:bg-slate-800"
              >
                <div>
                  <p className="text-sm font-bold text-slate-900 dark:text-slate-200 uppercase tracking-tight">{bet.betType} Bet</p>
                  <p className="text-[11px] text-slate-500 font-medium">#{bet.id.slice(-8)} · {bet.numbers.join(', ')}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-black text-indigo-600 dark:text-indigo-400">₹{(bet.amount || 0).toLocaleString()}</p>
